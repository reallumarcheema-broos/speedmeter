...
// Search-engine ownership files live at the site root and are .html, but they
// are not pages: listing them in the sitemap invites crawlers to index a token.
const NOT_A_PAGE = /^(404|google[0-9a-z]+|BingSiteAuth|yandex_[0-9a-z]+)\.html$/i;

/** Lists every real page as a clean URL, used for the sitemap. */
function listPages(root, dir = root, prefix = '') {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    } else if (entry.name.endsWith('.html') && !NOT_A_PAGE.test(entry.name)) {
